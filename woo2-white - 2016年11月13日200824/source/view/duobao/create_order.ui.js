 ...
 */
var app,page,core,goods_id,dataCache,kess,global,token,algorithm;
app = sm('do_App');
page = sm('do_Page');
...
(function(a6,a7,a8){
	a6.on('touch',function(){
		...
		ui('do_Label_27').text = (price*num).toFixed(2);
		scoreAct = true;
	});
	...
	a8.on('touch',function(){
		...
		ui('do_Label_30').visible = true;
			...
			if(next_score_tmp>(price*num)){
				...
				ui('do_Label_27').text = price*num*0.6;
				buy_score = 0;
				return_score = 0;
...
token = algorithm.des3Sync('woo','encrypt',kess.lockIt(userInfo.data.id));
http.url = "http://192.168.1.167:8099/index.php/index/panicbuy/order/token/"+token;
http.method = "POST";
...
//http2.url = "http://192.168.1.167:8099/index.php/index/panicbuy/order_create";
//http2.method = "POST";
...
ui('do_Button_1').on('touch','',3000,function(){
	...
	var payUrl = "http://192.168.1.167:8099/index.php/index/panicbuy/order_create";
	payUrls = payUrl+'?userId='+userInfo.data.id+'&address_id='+address_id+'&postscript='+postscript+"&sales_id="+goods_id+"&sales_num="+num+"&sales_total_price="+(price*num).toFixed(2)+"&buy_score="+buy_score+"&return_score="+return_score+"&next_score="+next_score+"&point="+point;
//	http2.body = {
