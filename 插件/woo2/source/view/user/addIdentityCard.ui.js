...
do_Button_1.on('touch',function(){
	...
	http.contentType = "application/json"; 
	http.url = "http://api.e-shy.com/index.php/index/Identity_card/IdentityCard"; 
	postData = {
	...
	http.on("success", function(result) {
		...
		do_ProgressBar_1.visible = false;
		if(result.code == 1){
			storage.writeFile("data://userInfo",result,function(boo){
				if(boo){
...
http3.url = "http://api.e-shy.com/index.php/index/File_upload/uploadImage";
